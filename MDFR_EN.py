...
# Streamlit 应用程序标题
st.title('Multidirectional Balance Function Assessment')
...
# 用户输入部分
st.sidebar.header('OpenCap Data Input')
uploaded_file = st.sidebar.file_uploader("trc files。", type=["trc"])

# 展示按钮设置
display_button = st.sidebar.button("Display")

# 示例展示按钮设置
test_display_button = st.sidebar.button("Example Display")
...
<div class="box">
    <h4>OpenCap Shooting Guidelines</h4>
    <ul>
        <li>Ensure that the subject being filmed is fully visible in the frame.</li>
        <li>Try to avoid cluttered backgrounds during filming.</li>
        <li>For the balance function test, the subject should maintain a still posture (it is recommended that the subject stands with feet together) before starting OpenCap data collection.</li>
        <li>After the OpenCap data collection begins, the subject should maintain a still posture for at least 3 seconds before starting the balance function test.</li>
        <li>The subject must keep both feet on the ground until OpenCap data collection is complete.</li>
    </ul>
</div>
""", unsafe_allow_html=True)
...
def calculate_and_display(joint_data):
    ...
    st.markdown(
        ...
        <div style="border: 2px solid green; padding: 10px; border-radius: 10px;">
            <h3>Maximum Displacement Information Relative to Initial Center of Mass</h3>
            <div style="display: flex; justify-content: space-around;">
                <div>Maximum Forward Displacement:<br>{:.4f} m</div>
                <div>Maximum Backward Displacement:<br>{:.4f} m</div>
            </div>
            <div style="display: flex; justify-content: space-around;">
                <div>Maximum Left Displacement:<br>{:.4f} m</div>
                <div>Maximum Right Displacement:<br>{:.4f} m</div>
            </div>
        </div>
    ...
    hover_text = [
        "Time: {:.2f} s<br>Height: {:.4f} m<br>Left-Right: {:.4f} m<br>Forward-Backward: {:.4f} m".format(
    ...
    scatter_layout = go.Layout(
        title= {
        'text': '3D Scatter Plot of Center of Mass Displacement',
        'y': 0.9,  # position title higher
        'x': 0.5,  # center title
        'xanchor': 'center',
        'yanchor': 'top',
        'font': {
        ...
        scene=dict(
            xaxis=dict(title='Left-Right (m)', range=[transformed_mass_df['左右'].min(), transformed_mass_df['左右'].max()]),
            yaxis=dict(title='Height (m)', range=[transformed_mass_df['高低'].min(), transformed_mass_df['高低'].max()]),
            zaxis=dict(title='Forward-Backward(m)', range=[transformed_mass_df['前后'].min(), transformed_mass_df['前后'].max()]),
            aspectmode='cube',
            camera=dict(
    ...
    st.write('This image can be rotated to adjust the viewing angle with the mouse. The origin of the coordinates is defined as the midpoint between the feet in the spatial coordinate system. Each scatter point corresponds to the position of the center of mass at that moment within this coordinate system. The definitions for the positive and negative values are as follows: "Positive values indicate positions in front of the origin, negative values indicate positions behind; positive values indicate positions to the left of the origin, negative values indicate positions to the right; positive values indicate positions above the origin, negative values indicate positions below."')
    # 显示最大位移信息
